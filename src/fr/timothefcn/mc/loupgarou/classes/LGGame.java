...
public class LGGame implements Listener {
    ...
    @Getter
    private LGChat spectatorChat = new LGChat((sender, message) -> "§7" + sender.getName() + " §6» §f" + message);
    @Getter
    private LGChat dayChat = new LGChat((sender, message) -> "§e" + sender.getName() + " §6» §f" + message);
    @Getter
  ...
  /*      try {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (timeLeft == 5 * 2 - 1) {
                    for (LGPlayer lgp : getInGame()) {
                        lgp.sendMessage("§8Plugin original par : §e§lLeomelki§8.\n§8Adapté par : §e§lTimotheFCN§8.\n");
                        lgp.sendTitle("", "§8§8Plugin LoupGarou par §e§lLeomelki§8, §e§lShytoos §8 & §e§lTimotheFCN", 40);
                        lgp.getPlayer().getInventory().clear();
                        lgp.getPlayer().updateInventory();
                    }
    ...
    public void onCustomItemChange(LGCustomItemChangeEvent e) {
        if (e.getGame() == this) {
            if (getMayor() == e.getPlayer())
                e.getConstraints().add(LGCustomItems.LGCustomItemsConstraints.MAYOR.getName());
            if (e.getPlayer().isDead())
                e.getConstraints().add(LGCustomItems.LGCustomItemsConstraints.DEAD.getName());
        }
    }
