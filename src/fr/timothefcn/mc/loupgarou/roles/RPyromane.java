...
import fr.timothefcn.mc.loupgarou.events.LGEndCheckEvent;
import fr.timothefcn.mc.loupgarou.events.LGGameEndEvent;
import fr.timothefcn.mc.loupgarou.events.LGPlayerKilledEvent;
import fr.timothefcn.mc.loupgarou.events.LGPyromaneGasoilEvent;
            ...
            if (lgp.getCache().<List<LGPlayer>>get("pyromane_essence").size() != 0) {
                ...
                for (LGPlayer scndPlayer : liste) {
                    if (!scndPlayer.isDead() && scndPlayer.getPlayer() != null) {
                        getGame().kill(scndPlayer, LGPlayerKilledEvent.Reason.PYROMANE);
                    }
                }
                liste.clear();
