...
import fr.timothefcn.mc.loupgarou.classes.LGCustomItems;
...
import fr.timothefcn.mc.loupgarou.events.LGNightEndEvent;
...
public class RLoupGarouNoir extends Role {
    ...
    public boolean hasPlayersLeft() {
        return super.hasPlayersLeft() && getGame().getDeaths().containsKey(LGPlayerKilledEvent.Reason.LOUP_GAROU);
    }
    ...
    protected void onNightTurn(LGPlayer player, Runnable callback) {
        toInfect = getGame().getDeaths().get(LGPlayerKilledEvent.Reason.LOUP_GAROU);
    ...
    public void onInventoryClick(InventoryClickEvent e) {
        ...
        if (item.getItemMeta().getDisplayName().equals(items[3].getItemMeta().getDisplayName())) {
            ...
            lgp.stopChoosing();
            getGame().getDeaths().remove(LGPlayerKilledEvent.Reason.LOUP_GAROU, toInfect);
            lgp.hideView();
            callback.run();
        }
    }
    ...
    public void onCustomItemChange(LGCustomItemChangeEvent e) {
        if (e.getGame() == getGame())
            if (e.getPlayer().getCache().getBoolean("infected"))
                e.getConstraints().add(LGCustomItems.LGCustomItemsConstraints.INFECTED.getName());
    }
